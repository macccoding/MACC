...
function Lightbox({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`${item.coverImage ? "" : item.bg} relative flex aspect-[4/3] w-full items-center justify-center overflow-hidden rounded-lg border border-white/5`}
        >
          {item.coverImage ? (
            <Image
              src={item.coverImage}
          ...
          <ScrollReveal>
            <div className="mb-12 flex items-center justify-between border-b border-white/5 pb-6">
              <span className="font-[family-name:var(--font-jetbrains)] text-[11px] uppercase tracking-[0.2em] text-text-muted">
                SHOWING {filteredItems.length} OF {items.length} FRAMES
              </span>
              <span className="font-[family-name:var(--font-jetbrains)] text-[11px] uppercase tracking-[0.2em] text-text-muted">
                {activeCategory}
              </span>
            ...
            <AnimatePresence mode="popLayout">
              {filteredItems.map((item, index) => (
                ...
                >
                  ...
                  >
                    <div
                      className={`${item.aspect} ${item.coverImage ? "bg-bg-secondary" : item.bg} relative flex w-full items-center justify-center overflow-hidden border border-white/[0.04] transition-all duration-300 group-hover:border-accent/30 group-hover:ring-1 group-hover:ring-accent/20 group-hover:shadow-[0_10px_40px_rgba(229,184,32,0.08)]`}
                    >
                      {item.coverImage ? (
                        <Image
                          src={item.coverImage}
